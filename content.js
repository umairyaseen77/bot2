...
async function checkForNewJobs() {
  ...
  try {
    ...
    const { processedJobSignatures = [] } = await chrome.storage.local.get('processedJobSignatures');
    const processedSignatures = new Set(processedJobSignatures);
    ...
    if (newJobs.length > 0) {
      ...
      // Save updated signatures to storage
      await chrome.storage.local.set({ processedJobSignatures: Array.from(processedSignatures) });
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  switch (request.action) {
    ...
    case 'getStatus':
      chrome.storage.local.get('processedJobSignatures', (result) => {
        sendResponse({
          monitoring: isMonitoring,
          lastCheck: lastCheckTime,
          jobCount: result.processedJobSignatures ? result.processedJobSignatures.length : 0
        });
      });
